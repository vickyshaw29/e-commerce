...
// action for checking the order as delivered by the admin
export const deliverOrder=(order)=>async (dispatch,getState)=>{
    try {
        ...
        const {data}=await axios.put(`http://localhost:8000/api/order/${order._id}/delivered`,{},config)
        // console.log(data,"data from actions")
        dispatch({
...
// action for getting all the orders by the admin
export const getOrderByAdmin=()=>async (dispatch,getState)=>{
    try {
        dispatch({
            type:ORDER_ADMIN_MY_REQUEST,
        })
        const {userLogin:{userInfo}}=getState()
        const config={
            headers:{
        ...
        const {data}=await axios.get('http://localhost:8000/api/allorders',config)
        dispatch({
            type:ORDER_ADMIN_MY_SUCCESS,
            payload:data
        })
    } catch (error) {
        dispatch({
            type:ORDER_ADMIN_MY_FAIL,
            payload:error
        })
    }
}